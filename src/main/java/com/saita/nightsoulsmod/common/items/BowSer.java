...
public class BowSer extends ShootableItem implements IVanishable {
	   public BowSer(Item.Properties builder) {
		      super(builder);
	}
		   ...
		   public void onPlayerStoppedUsing(ItemStack stack, World worldIn, LivingEntity entityLiving, int timeLeft) {
		      if (entityLiving instanceof PlayerEntity) {
	            ...
	            playerentity.playSound(SoundInit.BOW_SER.get(), 1.0F, 1.0F);
	            int randomLaugh = random.nextInt(6);
	            if(randomLaugh == 0)
	            {
	            	playerentity.playSound(SoundInit.BOWSER_LAUGH.get(), 1.0F, 1.0F); // Sound effect
	            }
		         ...
		         if (!itemstack.isEmpty() || flag) {
		            ...
		            if (!((double)f < 0.1D)) {
		               ...
		               if (!worldIn.isRemote) {
		                  ...
		                  abstractarrowentity.setDamage(12.5D); //Raw Damage Here
		                  if (f == 1.0F) {
		                  ...
		                  if (j > 0) {
		                     abstractarrowentity.setDamage(abstractarrowentity.getDamage() + (double)j * 1.1D + 0.5D); //Power Damage Here
		                  }

		                  int k = EnchantmentHelper.getEnchantmentLevel(Enchantments.PUNCH, stack);
		                  if (k > 0) {
		                     abstractarrowentity.setKnockbackStrength(k + 2); //Knockback strength
		                  }
		                  
		                  abstractarrowentity.setFire(150); // Arrow always on fire

		                  if (EnchantmentHelper.getEnchantmentLevel(Enchantments.FLAME, stack) > 0) {
		                     abstractarrowentity.setFire(300); // Fire effect increases when flame enchant
		                  }
		   ...
		   public static float getArrowVelocity(int charge) {
		      float f = (float)charge / 12.0F;  //Speed here. Default Bow is 20.
		      f = (f * f + f * 2.0F) / 3.0F;
		      if (f > 1.0F) {
		         f = 1.0F;
		      }
