...
public class MiizealiteBow extends ShootableItem implements IVanishable {
	   public MiizealiteBow(Item.Properties builder) {
		      super(builder);
	}
		         ...
		         if (!itemstack.isEmpty() || flag) {
		            ...
		            if (!((double)f < 0.1D)) {
		               ...
		               if (!worldIn.isRemote) {
		                  ...
		                  abstractarrowentity.setDamage(3.0D); //Raw Damage Here
		                  if (f == 1.0F) {
		                  ...
		                  if (j > 0) {
		                     abstractarrowentity.setDamage(abstractarrowentity.getDamage() + (double)j * 0.65D + 0.5D); //Power Damage Here
		                  }
		   ...
		   public static float getArrowVelocity(int charge) {
		      float f = (float)charge / 20.0F;  //Speed here. Default Bow is 20.
		      f = (f * f + f * 2.0F) / 3.0F;
		      if (f > 1.0F) {
		         f = 1.0F;
		      }
